...
const getDocumentById= asyncHandler(async doc => {
    ...
    res.status(201).json({message:'sucessfully updated document', document})
})

const updateDocument = asyncHandler(async doc => {
    await doc.findByIdAndUpdate(req.params.id,req.body);
    res.status(201).json({message:'sucessfully created document'})
})
