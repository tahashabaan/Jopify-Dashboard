...
() => {
    ...
    it('check user exist or not', async() => {
        const res = await request(app).get('/user').set('Authorization', `Barear ${__token}`).send(user)
        expect(res.status).toBe(201)
    })
})
...
() => {
    it('check user exist or not by id', async() => {
        const res = await request(app).get(`/user/${__id}`).set('Authorization', `Barear ${__token}`)
        expect(res.status).toBe(201)
    })

    it('check update user by id', async() => {
        const res = await request(app).put(`/user/${__id}`).set('Authorization', `Barear ${__token}`)
        expect(res.status).toBe(201)
    })
